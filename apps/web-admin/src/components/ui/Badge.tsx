...
/** Solid badge — reserved for counts / SLA-breach urgency, never for regular status. */
...
export function BadgeSkeleton({ width = 92 }: { width?: number }) {
  return (
    <span
      className="inline-block h-[26px] animate-[lalaShimmer_1.4s_linear_infinite] rounded-full bg-quiet-bg bg-[length:240%_100%]"
      style={{
        width,
        backgroundImage:
          "linear-gradient(90deg, var(--color-quiet-bg) 25%, color-mix(in srgb, var(--color-quiet-bg) 55%, var(--color-surface)) 50%, var(--color-quiet-bg) 75%)",
      }}
      aria-hidden="true"
    />
  );
